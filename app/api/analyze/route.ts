...
interface Pass1Data {
  ...
  pass1Identifications: Pass1Identification[];
  category:           string;
  condition:          string;
  materials:          string;
...
export async function POST(request: NextRequest) {
  try {
    const { imageBase64, mediaType, mode, label, highResBase64 } =
    ...
    if (mode === 'detect') {
      if (!imageBase64 || !mediaType) {
        return Response.json({ error: 'imageBase64 and mediaType required for detect mode' }, { status: 400 });
      }
        ...
        messages: [
          {
            ...
            content: [
              ...
              {
                type: 'text',
                text: `You are an expert appraiser and identifier scanning an image for anything worth identifying or listing.
...
Be thorough. Return between 1 and 10 subjects. Do not merge multiple distinct subjects into one box.`,
      ...
      const raw     = response.content.map((b) => ('text' in b ? b.text : '')).join('').replace(/```json|```/g, '').trim();
      const objects = JSON.parse(raw) as DetectedObject[];
    ...
    if (mode === 'list') {
      if (!label) {
        return Response.json({ error: 'label required for list mode' }, { status: 400 });
      }

      const imageData = highResBase64 || imageBase64;
      if (!imageData) {
        return Response.json({ error: 'imageBase64 or highResBase64 required for list mode' }, { status: 400 });
      }

      // ── Pass 1: visual identification ──
        ...
        messages: [
          {
            ...
            content: [
              ...
              {
                type: 'text',
                text: `You are a world-class expert identifier and appraiser. Identify this item as specifically and accurately as possible.

The item label provided is: "${label}" but do NOT rely on this. Look at the image with fresh eyes.

IDENTIFICATION METHODOLOGY:

1. VISUAL INVENTORY: List every visual detail including shape, proportions, colors, materials, hardware, markings, logos, labels, signatures, wear patterns, construction method, style indicators.
...
Return ONLY valid JSON:
{
  ...
  "estimatedDimensions": "Size estimate based on proportions"
      ...
      let pass1Data: Pass1Data;
      try {
        const raw1 = pass1.content.map((b) => ('text' in b ? b.text : '')).join('').replace(/```json|```/g, '').trim();
        pass1Data  = JSON.parse(raw1) as Pass1Data;
      } catch {
        pass1Data = {
      ...
      const googleContext = googleResults.length > 0
        ? googleResults.map((r, i) => `Result ${i + 1}: ${r.title} - ${r.snippet}`).join('\n')
        : 'No Google results found.';

      // ── Pass 2: Google-verified pricing + listing ──
        ...
        messages: [
          {
            ...
            content: [
              ...
              {
                type: 'text',
                text: `You are verifying and finalizing an identification.

First pass identified this as: "${topId}"
Visual inventory: ${pass1Data.visualInventory}
Brand markings: ${pass1Data.brandMarkings}

GOOGLE SEARCH RESULTS for "${topId} resale price used":
${googleContext}

YOUR TASK:
1. Look at the image again carefully
2. Use the Google results to CONFIRM or CORRECT the identification
3. Use Google results to inform realistic pricing based on actual market data
4. Assign a final confidence score 0-100
5. Write a compelling accurate listing

Pricing guidelines:
- Base pricing on Google search results if they show actual sold prices or listings
- Furniture: 20-35% of retail
...
{
  "identifications": [
    { "name": "Final most specific identification confirmed by visual evidence and Google results", "confidence": "high", "reasoning": "combined evidence from visual analysis and web results" },
  ...
  "listing": "4 paragraph professional listing. Para 1: precise identification and standout features. Para 2: materials, construction, breed or species details, or model specifics. Para 3: honest condition assessment with any flaws noted. Para 4: dimensions estimate and relevant care or logistics info."
      ...
      try {
        const raw2 = pass2.content.map((b) => ('text' in b ? b.text : '')).join('').replace(/```json|```/g, '').trim();
        result     = JSON.parse(raw2) as Record<string, unknown>;
      } catch {
        result = {
          identifications: pass1Data.pass1Identifications?.map((i) => ({
            name:       i.name,
            confidence: i.confidence,
            reasoning:  i.evidence,
          })) ?? [{ name: label, confidence: 'medium', reasoning: 'Parse error' }],
          confidenceScore:     50,
          condition:           pass1Data.condition,
          materials:           pass1Data.materials,
          estimatedDimensions: pass1Data.estimatedDimensions,
